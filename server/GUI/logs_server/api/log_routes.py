...
def update_log_entry(log_id: int, log: schemas.LogCreate, db: Session = Depends(get_db)):
    updated_log = log_crud.update_log(db, log_id, log)
    if not updated_log:
        raise HTTPException(status_code=404, detail="Log not found")
    return updated_log
