...
export const useAppStore = create<AppState & AppActions>()(
  immer((set, get) => ({
    ...
    clearAnnotation: (url) => {
      set(state => {
        ...
        if (source) {
          ...
          source.annotation.zoneType = null; // Reset zoneType
    ...
    startAnalysis: async (url) => {
      ...
      } else if (!source.annotation.zoneType) { // Check if zoneType is set
         ...
         set(state => {
            if(state.rtspSources[url]) {
                ...
                state.rtspSources[url].errorMessage = errorMessage;
                 // Keep mode as line_selection if polygon is closed
      ...
      // Check for zoneType which should not be null here based on above validation
    ...
    setZoneType: (url, type) => { // Rename action to setZoneType
        set(state => {
            const source = state.rtspSources[url];
            // Can only set zone type if polygon is closed
            if (source && source.annotation.isClosed) {
